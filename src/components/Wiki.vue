    ...
    <div class="m-wiki m-post m-block" v-loading="loading">
        <ul class="u-list" v-if="data.length">
            <li class="u-item" v-for="(item, i) in data" :key="i">
                ...
                >
                <span class="u-link"
                    ><time class="u-date">{{ formatDate(item.updated) }} @ {{ item.user_nickname || $t('匿名') }}</time><span class="u-type">{{ formatType(item.type) }} » {{ item.source.name }} › {{item.remark}}</span></span
                >
            </li>
        </ul>
        <el-alert v-else class="m-archive-null" :title="$t('没有找到相关条目')" type="info" center show-icon> </el-alert>
        <el-button class="m-archive-more" type="primary" :class="{ show: hasNextPage }" :loading="loading" @click="appendPage(++page)">{{ $t('加载更多') }}</el-button>
